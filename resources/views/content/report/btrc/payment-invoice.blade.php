...
@php $route = 'Payment Invoice' @endphp
@section('title', $route)
...
<div class="card">
  <div class="card-header py-1">
    <h5 class="card-title">Payment Invoice</h5>

    <form action='{{route("reprot.payment-invoice")}}' class="">
      <div class="row">
    ...
    <table id="example" class="display nowrap datatables-users table">
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>{{$item->updated_at ? $item->updated_at->format('d-m-Y h:i:s a') :'N/A'}} </td>
          <td>{{$item->invoice_no }}</td>
          <td>{{$item->customer?$item->customer->username:'N/A'}}</td>
          <td>{{$item->package ? $item->package->name:'N/A'}}</td>
          <td>{{$item->amount ? $item->amount :'00'}}</td>
          <td>{{$item->received_amount ? $item->received_amount :'00'}}</td>
          <td>{{$item->paid_by}}</td>
